...
<body>
	...
	if (isset($_POST['envoyer'])) 
	{
		if (isset($_POST['email']) AND !empty($_POST['email'])) 
		{
			...
			$usersbdd = $bdd->prepare("SELECT * from users_admins WHERE email = ?");
			...
			{
				$token = uniqid();
				$url = "http://crypto.boss-arts.com/admin/nouveau-mot-de-passe-admin.php?email=$email&token=$token";
			...
			// Variables concernant l'email
				...
				if ($mail) 
				{
					$inserttoken = $bdd->prepare("UPDATE users_admins SET token = ? WHERE email = ?");
