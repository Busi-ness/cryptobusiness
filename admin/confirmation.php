...
<body>
	...
	<div class="menu">
...

		{
			...
			$requser = $bdd->prepare("SELECT * FROM users_admins WHERE email = ? AND confirmkey = ?");
			...
			if ($userexist == 1) 
			{
				...
				if ($user['confirme'] == 0) 
				{
					$updateuser = $bdd->prepare("UPDATE users_admins SET confirme = 1 WHERE email = ? AND confirmkey = ?");
					$updateuser->execute(array($email, $confirmkey));

					$note = "Je vous remercie pour l'activation de votre compte, vous pouvez maintenant vous connecter <a href='http://crypto.boss-arts.com/admin/connexion.php'>ici</a>";

				}else
				{
			//else de l'utilisateur a déjà confirmé son compte
					$note = "Votre compte a déjà été confirmé ! Connectez vous <a href='http://crypto.boss-arts.com/admin/connexion.php'>ici</a>";
				}
