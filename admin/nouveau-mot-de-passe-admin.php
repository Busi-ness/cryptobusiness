...
<body>
	...
	if (isset($_GET['email']) AND !empty($_GET['email']) AND isset($_GET['token']) AND !empty($_GET['token']) ) 
	{
		...
		$usersbdd1= $bdd->prepare("SELECT * from users_admins WHERE email = ?");
		$usersbdd1->execute(array($email));
		$userexist1 = $usersbdd1->rowcount();

		if ($userexist1 == 1) 
		{
			$usersbdd2 = $bdd->prepare("SELECT * from users_admins WHERE token = ?");
...
if (isset($_POST['confirmer'])) 
{
	...
	if (isset($_POST['password']) AND !empty($_POST['password'])) 
	{
		...
		$tokenbdd= $bdd->prepare("UPDATE users_admins SET password = ?, token = NULL");
...
<div class="note">
	...
	if (isset($note)) 
	{
		...
		echo $note;
		?>
		<a href="http://crypto.boss-arts.com/admin/connexion.php"><button>Connectez-vous ici</button></a>	
	<?php
}
?>
</div>
