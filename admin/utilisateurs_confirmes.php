...
<html lang="fr">
<head>
	<title>Utilisateurs confirmés| Crypto Business</title>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="stylesheet" href="css_perso/style.css">
	<link rel="stylesheet" href="css_perso/utilisateurs_confirmes.css">
		...
		if ($getid = $userinfo['id']) 
		{
			if (isset($_SESSION['id']) AND !empty($_SESSION['id'])) 
			{
				...
				if ($getid == $_SESSION['id']) 
				{
					...
					$users = $bdd->prepare('SELECT * from users WHERE confirme = 1');
					$users->execute(array());
					...
					<!-- En tête du tableau -->
					<div>
						<h2>Les utilisateurs confirmés</h2>
					</div>
					<br/>
